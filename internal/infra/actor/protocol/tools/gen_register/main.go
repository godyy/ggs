...
func generateRegisterFile(pkgPath string, registryMethod string, items []enumItem) ([]byte, error) {
	...
	for _, it := range items {
		...
		buf.WriteString(fmt.Sprintf("\t%s(%s.PID_%s, &%s.%s{})\n", registryMethod, pkgName, name, msgPkg, msg))
	}
	buf.WriteString("}\n")
...
func main() {
	...
	flag.Parse()
	if *protoRoot == "" {
		panic(fmt.Errorf("missing -proto-path flag: set path to ggs/app/internal/protocol"))
	}
	...
	// Write outputs
	if err := os.WriteFile(filepath.Join(registryDir, "c2s_register.go"), c2sCode, 0o644); err != nil {
		panic(err)
	}
	if err := os.WriteFile(filepath.Join(registryDir, "s2s_register.go"), s2sCode, 0o644); err != nil {
		panic(err)
	}
}
