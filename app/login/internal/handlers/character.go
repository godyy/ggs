...
func (h *characterHandler) genLoginAgentToken(tokenInfo *models.TokenInfo) (string, error) {
	sub, err := json.Marshal(tokenInfo)
	if err != nil {
		return "", err
	}

	token, err := authjwt.SignToken(h.getSignKey(), app.Env().Stage(), string(sub), time.Minute*5, time.Now())
	if err != nil {
		return "", err
	}
