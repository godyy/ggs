...
import (
	...
	gameconsts "github.com/godyy/ggs/app/game/internal/base/consts"
...
func (p *Player) OnStart() error {
	...
	// 加载model数据
	if err := LoadActorModel(p); err != nil {
		return err
	}
...
func (p *Player) OnStop() error {
	...
	if ok, _ := p.Model.IsDirty(); ok {
		if err := SaveActorModel(p); err != nil {
			// todo 日志
		}
	}
...
func (p *Player) OnModelDirty() {
	...
	DelaySaveActor(p, gameconsts.ActorSaveDelay)
}
